import { BarChart2, Code, Database, Filter, PenTool } from "lucide-react";
...
function JobCategories({number, clickedFilter}: Props) {
  const categories = [
    ...
    { icon: <Database size={24} />, title: 'Database', count: '1.3k Postings' },
    { icon: <Filter size={24} />, title: 'Unfilter', count: '2.0k Postings' },
  ];
  ...
  return (
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-7xl mx-auto px-4 mb-16">
      { categories.map((category, index) => ( 
        ...
        // Every category will invoke the callback function and set state with the category value it has
